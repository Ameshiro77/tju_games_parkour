execute unless score @s 9_passed matches 1 run scoreboard players add @s rank 1
...
execute if score @s end >= @s level_9 run tp @s 0 22 0
...
#记录提示
execute as @a[x=-17,y=29,z=72,dx=0,dy=1,dz=0] if score @s level_9 > @s end run tellraw @s ["您刷新了个人该关卡最快记录!本次共用时:",{"score":{"name":"@s","objective":"end"} },"s!"]
#此时：如果end>limit，那么不加分，tp
execute as @a[x=-17,y=29,z=72,dx=0,dy=1,dz=0] if score @s end > #limit_9 limit run tellraw @a [{"text":"恭喜","color":"green"},{"selector":"@s","color": "aqua"},{"text":"通过第9关!","color":"green"}]
execute as @a[x=-17,y=29,z=72,dx=0,dy=1,dz=0] if score @s end > #limit_9 limit run spawnpoint @s -1 22 0
execute as @a[x=-17,y=29,z=72,dx=0,dy=1,dz=0] if score @s end > #limit_9 limit run scoreboard players operation @s level_9 < @s end
#tp:
execute as @a[x=-17,y=29,z=72,dx=0,dy=1,dz=0] if score @s end > #limit_9 limit run tp @s 0 22 0
#如果：end进了limit，但是level_x没进，那么limit-
#如果：end和level_x都进了，那么level_x减
#因此，我们用已经没用的start保存更小值
execute as @a[x=-17,y=29,z=72,dx=0,dy=1,dz=0] run scoreboard players set @s start 2147483647
execute as @a[x=-17,y=29,z=72,dx=0,dy=1,dz=0] run scoreboard players operation @s start < #limit_9 limit
execute as @a[x=-17,y=29,z=72,dx=0,dy=1,dz=0] run scoreboard players operation @s start < @s level_9
#然后start-end,得到进步的值  必须是线性的
execute as @a[x=-17,y=29,z=72,dx=0,dy=1,dz=0] run scoreboard players operation @s start -= @s end
#最后除以倍率，存到start
...
#以前结束，传送回去
execute as @a[x=-17,y=29,z=72,dx=0,dy=1,dz=0] run tellraw @a [{"text":"恭喜","color":"green"},{"selector":"@s","color": "aqua"},{"text":"通过第9关!","color":"green"}]
execute as @a[x=-17,y=29,z=72,dx=0,dy=1,dz=0] run spawnpoint @s -1 22 0
execute as @a[x=-17,y=29,z=72,dx=0,dy=1,dz=0] run tp @s 0 22 0